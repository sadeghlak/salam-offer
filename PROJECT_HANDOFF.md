...
- `/` صفحه اصلی Salam Offer و داشبورد عملیاتی run-centric است، نه صرفاً overview اجراها.
- `dashboard.html` شامل sidebar اجراهای روزانه است؛ اجرای امروز براساس `business_date` به صورت پیش‌فرض انتخاب می‌شود و اگر نبود آخرین اجرا باز می‌شود.
- `/runs/<run_key>/` همان داشبورد را با اجرای انتخاب‌شده باز می‌کند تا لینک‌های قبلی نشکنند.
- محصول‌ها در داشبورد به صورت جدول فشرده و قابل اسکن نمایش داده می‌شوند، نه کارت‌های حجیم.
- dashboard دکمه تحلیل، وضعیت polling و خروجی اکسل گزارش تحلیل دارد.
